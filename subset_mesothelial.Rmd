...
# Find mesothelial population

```{r}
DimPlot(seurat, label=T)
FeaturePlot(seurat, features=c("Msln"))
```

```{r}
seurat <- subset(seurat, idents=16)
```
...
seurat.anchors <- FindIntegrationAnchors(seurat.list, 
                                         ...
                                         anchor.features = seurat.features,
                                         k.filter = 60, # <smallest sample size (80), otherwise error
                                         verbose = T)
...
Cluster 1 = Fibroblast doublets

```{r}
FeaturePlot(seurat, features="Fn1")
```

# Filter out contaminating populations
```{r}
seurat <- subset(seurat, idents=c(0))
```
...
# Save point
```{r}
saveRDS(seurat, "../../output/mesothelial_cells.rds")
```
...
# Save!
write.csv(result_table, file = "../../output/dsa_results_mesothelial_all.csv", row.names = F)
```
...
write.csv(result_table, file = "../../output/dsa_results_mesothelial_nox_development.csv", row.names = F)
```
...
write.csv(result_table, file = "../../output/dsa_results_mesothelial_hyx_development.csv", row.names = F)
```

# GSEA on all clusters by logFC

```{r}
result_table <- read.csv("../../output/dsa_results_mesothelial_all.csv")
```
...
```{r}
writexl::write_xlsx(fgsea_results, path = "../../output/dsa_gsea_mesothelial_results.xlsx")
```
