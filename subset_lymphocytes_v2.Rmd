...
# Find Lymphocyte populations

```{r}
DimPlot(seurat, label=T)
FeaturePlot(seurat, features=c("Cd3d", "Ms4a1"))
```

5,7

```{r}
seurat <- subset(seurat, idents=c(5,7))
```
...
markers <- FindAllMarkers(seurat, only.pos = T, logfc.threshold = 1.5)
```
...
```{r}
seurat <- subset(seurat, idents=c(0,1,2,3,4,5,7,8))
```
...
# Save point
```{r}
saveRDS(seurat, "../../output/lymphocyte_cells.rds")
```
...
# Save!
write.csv(result_table, file = "../../output/dsa_results_lymphocytes_all.csv", row.names = F)
```
...
write.csv(result_table, file = "../../output/dsa_results_lymphocytes_nox_development.csv", row.names = F)
```
...
write.csv(result_table, file = "../../output/dsa_results_lymphocytes_hyx_development.csv", row.names = F)
```

# GSEA on all clusters by logFC

```{r}
result_table <- read.csv("../../output/dsa_results_lymphocytes_all.csv")
```
...
```{r}
writexl::write_xlsx(fgsea_results, path = "../../output/dsa_gsea_lymphocyte_results.xlsx")
```
