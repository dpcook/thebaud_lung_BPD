...
# Find fibroblast populations

```{r}
DimPlot(seurat, label=T)
FeaturePlot(seurat, features="Col1a1")
```
0,3,8,12

(note: 16 is Col1a1+, but is clearly mesothelial cells based on Msln)

```{r}
seurat <- subset(seurat, idents=c(0,3,8,12))
```
...
Cluster 8 - Immune cell
Cluster 9 - Cell cycle regression artifact? Proliferating cells

```{r}
DimPlot(seurat, group.by="Phase")
FeaturePlot(seurat, features=c("Ptprc", "Sftpc"))
```

# Filter out contaminating populations
```{r}
seurat <- subset(seurat, idents=c(0,1,2,3,4,5,6))
```
...
DimPlot(seurat, group.by="Oxygen")
DimPlot(seurat, group.by="Age", split.by="Oxygen")
```

# Save point
```{r}
saveRDS(seurat, "../../output/stroma_cells.rds")
```
...
# Save!
write.csv(result_table, file = "../../output/dsa_results_stroma_all.csv", row.names = F)
```
...
write.csv(result_table, file = "../../output/dsa_results_stroma_nox_development.csv", row.names = F)
```
...
write.csv(result_table, file = "../../output/dsa_results_stroma_hyx_development.csv", row.names = F)
```

# GSEA on all clusters by logFC

```{r}
result_table <- read.csv("../../output/dsa_results_stroma_all.csv")
```
...
```{r}
writexl::write_xlsx(fgsea_results, path = "../../output/dsa_gsea_stroma_results.xlsx")
```
