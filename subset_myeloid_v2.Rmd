...
# Find Myeloid populations

```{r}
DimPlot(seurat, label=T)
FeaturePlot(seurat, features="Cd14")
```

4,6,9,13,18 (18 is not Cd14+, but is Mast cells)

```{r}
seurat <- subset(seurat, idents=c(4,6,9,13,18))
```
...
FeaturePlot(seurat, features=c("Cldn5", "Col1a1"))
```

# Filter out contaminating populations
```{r}
seurat <- subset(seurat, idents=c(0,1,2,3,4,5,8,9,10,13))
```
...
# Save point
```{r}
saveRDS(seurat, "../../output/myeloid_cells.rds")
```
...
# Save!
write.csv(result_table, file = "../../output/dsa_results_myeloid_all.csv", row.names = F)
```
...
write.csv(result_table, file = "../../output/dsa_results_myeloid_nox_development.csv", row.names = F)
```
...
write.csv(result_table, file = "../../output/dsa_results_myeloid_hyx_development.csv", row.names = F)
```

# GSEA on all clusters by logFC

```{r}
result_table <- read.csv("../../output/dsa_results_myeloid_all.csv")
```
...
```{r}
writexl::write_xlsx(fgsea_results, path = "../../output/dsa_gsea_myeloid_results.xlsx")
```
